...
@SpringBootTest(properties = 'fint.consumer.cache.disabled.personalmappe=true')
@AutoConfigureMockMvc
@ActiveProfiles("test")
class PersonalmappeControllerSpec extends Specification {
    ...
    "query": "mappeId/2020/42"
        ...
        def response = mockMvc.perform(
                get(RestEndpoints.PERSONALMAPPE + '/mappeid/2020/42')
                        .header('x-org-id', 'test.org')
                        .header('x-client', 'Spock'))

        then:
        response.andExpect(status().is2xxSuccessful()).andExpect(jsonPath('$.navn.fornavn').value(equalTo('Spock')))
    ...
    def "Verify that PUT by mappeId works"() {
        when:
        def response = mockMvc.perform(
                put(RestEndpoints.PERSONALMAPPE + '/mappeid/2020/22')
                        .header('x-org-id', 'test.org')
                        .header('x-client', 'Spock')
