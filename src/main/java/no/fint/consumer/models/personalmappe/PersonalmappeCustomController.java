...
@RestController
@CrossOrigin
@RequestMapping(path = RestEndpoints.PERSONALMAPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class PersonalmappeCustomController {
    @Autowired
    private PersonalmappeController personalmappeController;
    ...
    public PersonalmappeResource getPersonalmappeByMappeArSekvensnummer(
        ...
        return personalmappeController.getPersonalmappeByMappeId(ar + "/" + sekvensnummer, orgId, client);
    }

    @PutMapping("/mappeid/{ar}/{sekvensnummer}")
    public ResponseEntity putPersonalmappeByMappeArSekvensnummer(
            @PathVariable String ar,
            @PathVariable String sekvensnummer,
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestBody PersonalmappeResource body) {
        return personalmappeController.putPersonalmappeByMappeId(ar + "/" + sekvensnummer, orgId, client, body);
    }
